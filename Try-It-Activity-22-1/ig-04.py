...
import sys                     # type: ignore  # noqa: E402
import os                      # type: ignore  # noqa: E402
import warnings                # type: ignore  # noqa: E402
...
logging.getLogger('tensorflow.keras').disabled = False
logging.getLogger('tensorflow_core').disabled  = False
logging.getLogger('tensorflow_hub').disabled   = False
logging.getLogger('tensorflow-macos').disabled = False
logging.getLogger('tensorflow-metal').disabled = False
...
consoleError   = logging.FileHandler(filename='ig-Error.log', mode='a')
FutureWarning  = logging.FileHandler(filename='ig-Warnings.log', mode='a')
fhandler.      setFormatter(formatter)
consoleHandler.setFormatter(formatter)
consoleError.  setFormatter(formatter)
FutureWarning. setFormatter(formatter)
fhandler.      setLevel(logging.INFO)
consoleHandler.setLevel(logging.INFO)
consoleError.  setLevel(logging.ERROR)
FutureWarning. setLevel(logging.WARNING)
...
#--------------------------------------------------------------------------------------------------------------------------------------------
logger.info('Step  8: Displaying images and printing top-k predictions')
logger.info('Step  8: -------------------------------------')
for (name, img_tensor) in img_name_tensors.items():
  ...
  logger.info(f'Step  8: Top {len(imagenet_labels)} predictions for {name}:')
  logger.info('Step  8: -------------------------------------')
  logger.info(f'Step  8: Image shape       : {img_tensor.shape}')
  logger.info(f'Step  8: Image pixel range : {tf.reduce_min(img_tensor):0.2f} to {tf.reduce_max(img_tensor):0.2f}')
  logger.info(f'Step  8: Image pixel mean  : {tf.reduce_mean(img_tensor):0.2f}')
  logger.info(f'Step  8: Image pixel stddev: {tf.math.reduce_std(img_tensor):0.2f}')
  logger.info('Step  8: Top predictions:')
  for label, prob in zip(pred_label, pred_prob):
    logger.info(f'Step  8: {label:<18}: {prob:0.1%}')
  logger.info('Step  8: -------------------------------------')
#-------------------------------------------------------------------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------------------------------------------------------------------
logger.info('Step 20: Visualizing Integrated Gradients attributions for specific images')
logger.info('Step 99: End of Script') 
fhandler.close()
